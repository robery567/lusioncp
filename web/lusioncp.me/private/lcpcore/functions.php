...
function sanitize($var) {
	if (is_array($var)) {
		return 'The variable must be a string.';
	}
...
function build_url() {
	...
	$pathname = (isset($_SERVER)) ? $_SERVER['REQUEST_URI'] : getenv('REQUEST_URI');
	$getquery = (isset($_SERVER)) ? $_SERVER['QUERY_STRING'] : getenv('QUERY_STRING');
...
function server_ip($username) {
	...
	";
	$data = $db->query($query);
	$result = ($data->num_rows > 0) ? $data->fetch_row : [0];
	return $result[0];
}
